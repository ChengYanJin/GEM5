...
def run(bench, input_set, l2_size, l2_assoc, num_threads):
    ...
    cmd_run = 'build/ALPHA_MESI_Two_Level/gem5.opt -d ' + dir + ' configs/example/fs.py --cpu-type=timing --num-cpus=' \
...
def add_experiments(bench, input_set):
  #  add_experiment(bench, input_set, '256kB', 4, 4)
    add_experiment(bench, input_set, '512kB', 32, 4)
...
for input_set in input_sets:
   ...
   # add_tasks('facesim', input_set)
    add_experiments('ferret', input_set)
    add_experiments('fluidanimate', input_set)
    # add_tasks('freqmine', input_set)
    add_experiments('streamcluster', input_set)
    add_experiments('swaptions', input_set)
